...
QArtText generates QR codes with embedded human-readable text. The example below 
is a working QR code: version 11 at level L, `Mixed 5×8` on a plate,
scanning to `qarttext.pages.dev`.
...
Cox bought his free bits by appending random characters to the URL's `#` fragment. 
Taking them from the padding instead adds nothing to the URL at all: what is encoded 
is what you typed, character for character, unless you turn on alphanumeric encoding, 
which folds case and nothing else.
...
Three fonts, times four rendering styles make the twelve variants, laid out as a grid with 
fonts named across the top and styles running down. The styles are not labeled: 
plate against halo, and upright against inverted, are plain from the pictures, 
...
Rendering styles cross two choices:
...
In the example shown at the beginnig, the text was nudged right, off the placement 
the offset search picked, so the `Q` clears the dashed column beside it. And the 
last `t` was finished by hand: two modules flipped in the editor, at column 52, 
rows 18 and 19. Those two sit in the same Reed-Solomon codeword, so between 
them they cost one of the ten that block can lose and still be read. That is the
accounting the editor shows you as you click, and the reason it counts per
block rather than over the whole symbol.

The app icon is the same construction shrunk as far as it will go: twenty-nine
modules against this one's sixty-one, about a fifth of the area, reading `QR`
rather than the whole name. It is a working code too, and scanning it opens
this site. It is generated by the encoder in this repository rather than
drawn, which is checked on every build -- the script refuses to write an icon
that does not encode the address it is supposed to.

## Rotation

Readers establish orientation from the finder patterns, so a symbol can be
turned through any quarter turn and still scan. The *Rotation* option does
that: the text is laid out upright on the turned symbol, the solver works in
the standard frame underneath, and everything shown, edited and exported is
the turned symbol. At 0° nothing is different.

A quarter turn helps and hurts for two separate reasons. The zig-zag lays out
codewords in column pairs from the right-hand edge, so the payload's immovable
modules gather in columns on the right; on a turned symbol they run along the
top or bottom edge, parallel to the text rather than across it, and the text
can sit where it would otherwise have been pushed away from. Against that,
Reed–Solomon blocks are independent: a block's error-correction codewords are a
function of that block's data alone, so a target module can only be steered by
free bits of its own block. A band running along the column pairs touches
fewer codewords and concentrates its demand on fewer blocks, which can run out
...
"Clean" is what the search accepts: no stuck letter module, nothing touching a
stroke in the wrong colour, and 98.5% of the plate as asked. On its own, a
quarter turn buys a smaller symbol and pays for it in stuck letters, which is
why it is an option and not the default. Where both orientations come out
clean, 90° is smaller than 0° in 66 codes and larger in 26. Taking the better
of two orientations for each code does better than either: 0° or 270° gives 200
clean at a mean of version 13.65, and all four give 211 at 13.58, at four times
the search. An earlier measurement, which picked the orientation in advance by
counting immovable modules under the band, found rotation worse; that count is
a poor predictor, and the conclusion drawn from it was wrong.
...
Only one color can win a corner. The four cells are shared, so joining one diagonal necessarily
